...
namespace!(
    ...
    /// `Derivation`: The more specific forms of prov:Derivation (i.e., prov:Revision, prov:Quotation, prov:PrimarySource) should be asserted if they apply.
    Derivation, "Derivation",
    /// `Dictionary`: This concept allows for the provenance of the dictionary, but also of its constituents to be expressed. Such a notion of dictionary corresponds to a wide variety of concrete data structures, such as a maps or associative arrays.
    ...
    /// `Influence`: Because prov:Influence is a broad relation, its most specific subclasses (e.g. prov:Communication, prov:Delegation, prov:End, prov:Revision, etc.) should be used when applicable.
    Influence, "Influence",
    /// `Insertion`: 
    Insertion, "Insertion",
    ...
    /// ``: 
    aq, "aq",
    /// `asInBundle`: prov:asInBundle is used to specify which bundle the general entity of a prov:mentionOf property is described.<br><br>When :x prov:mentionOf :y and :y is described in Bundle :b, the triple :x prov:asInBundle :b is also asserted to cite the Bundle in which :y was described.
    ...
    /// `mentionOf`: prov:mentionOf is used to specialize an entity as described in another bundle. It is to be used in conjuction with prov:asInBundle.<br><br>prov:asInBundle is used to cite the Bundle in which the generalization was mentioned.
    mentionOf, "mentionOf",
    /// ``: A reference to the principal section of the PROV-DM document that describes this concept.
    n, "n",
    ...
    /// ``: This annotation property links a subproperty of prov:wasInfluencedBy with the subclass of prov:Influence and the qualifying property that are used to qualify it. <br><br>Example annotation:<br><br>    prov:wasGeneratedBy prov:qualifiedForm prov:qualifiedGeneration, prov:Generation .<br><br>Then this unqualified assertion:<br><br>    :entity1 prov:wasGeneratedBy :activity1 .<br><br>can be qualified by adding:<br><br>   :entity1 prov:qualifiedGeneration :entity1Gen .<br>   :entity1Gen <br>       a prov:Generation, prov:Influence;<br>       prov:activity :activity1;<br>       :customValue 1337 .<br><br>Note how the value of the unqualified influence (prov:wasGeneratedBy :activity1) is mirrored as the value of the prov:activity (or prov:entity, or prov:agent) property on the influence class.
