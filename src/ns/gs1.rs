...
namespace!(
    "https://gs1.org/voc/",;
    /// `GS1 Application Identifier`: An application identifier defined by GS1, such as "01" for GTIN, "10" for batch/lot etc. Although they are all numeric, they are strings, not integers. Each AI has an associated value space that typically restricts the characters that can be used, the length and, in some cases a checkbit. The full list of AIs is maintained at https://www.gs1.org/standards/barcodes/application-identifiers.
    AI, "AI",
    ...
    /// `Due date for amount on payment slip`: The date by which an invoice should be paid. This data element represents an attribute of a payment slip reference number, AI (8020), and a Global Location Number (GLN) of the invoicing party.
    dueDate, "dueDate",
    ...
    /// `Link type for GS1 Digital Link`: Provides a URL for related information or services. This is not expected to be used directly but provides a super property for all other link types in the GS1 ecosystem.
    linkType, "linkType",
    ...
    /// `Production Date`: The production date is the production or assembly date determined by the manufacturer. The date may refer to the trade item itself or to items contained.
    productionDate, "productionDate",
    /// `Date and time of production`: The date and time of production is determined by the manufacturer. The date and time may refer to the trade item itself or to the items contained.
