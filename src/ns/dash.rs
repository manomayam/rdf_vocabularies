...
namespace!(
    ...
    /// `Action test case`: A test case that evaluates a dash:Action using provided input parameters. Requires exactly one value for dash:action and will operate on the test case's graph (with imports) as both data and shapes graph.<br><br>Currently only supports read-only actions, allowing the comparison of actual results with the expected results.
    ...
    /// `Boolean select editor`: An editor for boolean literals, rendering as a select box with values true and false.<br><br>Also displays the current value (such as "1"^^xsd:boolean), but only allows to switch to true or false.
    ...
    /// `Hyperlink viewer`: A Viewer for literals, rendering as a hyperlink to a URL.<br><br>For literals it assumes the lexical form is the URL.<br><br>This is often used as default viewer for xsd:anyURI literals. Unsupported for blank nodes.
    ...
    IDRole, "IDRole",
    /// `Icon`: Icon properties produce images that are typically small and almost square-shaped, and that may be displayed in the upper left corner of a focus node's display. Values should be xsd:string or xsd:anyURI literals or IRI nodes pointing at URLs. Those URLs should ideally be vector graphics such as .svg files.<br><br>Instances of the same class often have the same icon, and this icon may be computed using a sh:values rule or as sh:defaultValue.<br><br>If the value is a relative URL then those should be resolved against the server that delivered the surrounding page.
    IconRole, "IconRole",
    ...
    /// `List shape`: Defines constraints on what it means for a node to be a well-formed RDF list.<br><br>The focus node must either be rdf:nil or not recursive. Furthermore, this shape uses dash:ListNodeShape as a "helper" to walk through all members of the whole list (including itself).
    ListShape, "ListShape",
    ...
    Script, "Script",
    /// `Script constraint`: The class of constraints that are based on Scripts. Depending on whether dash:onAllValues is set to true, these scripts can access the following pre-assigned variables:<br><br>- focusNode: the focus node of the constraint (a NamedNode)<br>- if dash:onAllValues is not true: value: the current value node (e.g. a JavaScript string for xsd:string literals, a number for numeric literals or true or false for xsd:boolean literals. All other literals become LiteralNodes, and non-literals become instances of NamedNode)<br>- if dash:onAllValues is true: values: an array of current value nodes, as above.<br><br>If the expression returns an array then each array member will be mapped to one validation result, following the mapping rules below.<br><br>For string results, a validation result will use the string as sh:message.<br>For boolean results, a validation result will be produced if the result is false (true means no violation).<br><br>For object results, a validation result will be produced using the value of the field "message" of the object as result message. If the field "value" has a value then this will become the sh:value in the violation.<br><br>Unless another sh:message has been directly returned, the sh:message of the dash:ScriptConstraint will be used, similar to sh:message at SPARQL Constraints. These sh:messages can access the values {$focusNode}, ${value} etc as template variables.
    ...
    /// `Script function`: Script functions can be used from SPARQL queries and will be injected into the generated prefix object (in JavaScript, for ADS scripts). The dash:js will be inserted into a generated JavaScript function and therefore needs to use the return keyword to produce results. These JS snippets can access the parameter values based on the local name of the sh:Parameter's path. For example ex:value can be accessed using value.<br><br>SPARQL use note: Since these functions may be used from any data graph and any shapes graph, they must not rely on any API apart from what's available in the shapes graph that holds the rdf:type triple of the function itself. In other words, at execution time from SPARQL, the ADS shapes graph will be the home graph of the function's declaration.
    ScriptFunction, "ScriptFunction",
    /// `Script suggestion generator`: A Suggestion Generator that is backed by an Active Data Shapes script. The script needs to return a JSON object or an array of JSON objects if it shall generate multiple suggestions. It may also return null to indicate that nothing was suggested. Note that the whole script is evaluated as a (JavaScript) expression, and those will use the last value as result. So simply putting an object at the end of your script should do. Alternatively, define the bulk of the operation as a function and simply call that function in the script.<br><br>Each response object can have the following fields:<br><br>{<br>	message: "The human readable message",  // Defaults to the rdfs:label(s) of the suggestion generator<br>	add: [ // An array of triples to add, each triple as an array with three nodes<br>		[ subject, predicate, object ],<br>		[ ... ]<br>	],<br>	delete: [<br>		... like add, for the triples to delete<br>	]<br>}<br><br>Suggestions with neither added nor deleted triples will be discarded.<br><br>At execution time, the script operates on the data graph as the active graph, with the following pre-bound variables:<br>- focusNode: the NamedNode that is the sh:focusNode of the validation result<br>- predicate: the NamedNode representing the predicate of the validation result, assuming sh:resultPath is a URI<br>- value: the value node from the validation result's sh:value, cast into the most suitable JS object<br>- the other pre-bound variables for the parameters of the constraint, e.g. in a sh:maxCount constraint it would be maxCount<br><br>The script will be executed in read-only mode, i.e. it cannot modify the graph.<br><br>Example with dash:js:<br><br>({<br>	message: `Copy labels into ${graph.localName(predicate)}`,<br>	add: focusNode.values(rdfs.label).map(label => <br>		[ focusNode, predicate, label ]<br>    )<br>})
    ScriptSuggestionGenerator, "ScriptSuggestionGenerator",
    /// `Script test case`: A test case that evaluates a script. Requires exactly one value for dash:js and will operate on the test case's graph (with imports) as both data and shapes graph.<br><br>Supports read-only scripts only at this stage.
    ScriptTestCase, "ScriptTestCase",
    /// `Script validator`: A SHACL validator based on an Active Data Shapes script.<br><br>See the comment at dash:ScriptConstraint for the basic evaluation approach. Note that in addition to focusNode and value/values, the script can access pre-bound variables for each declared argument of the constraint component.
    ...
    /// `Single line constraint component`: A constraint component that can be used to declare that all values that are literals must have a lexical form that contains no line breaks ('\<br>' or '\').<br><br>User interfaces may use the dash:singleLine flag to prefer a text field over a (multi-line) text area.
    ...
    /// `Text field editor`: A simple input field to enter the value of a literal, without the ability to change language or datatype.<br><br>This is the fallback editor for any literal if no other editors are more suitable.
    ...
    /// `details endpoint`: Can be used to link a SHACL property shape with the URL of a SPARQL endpoint that may contain further RDF triples for the value nodes delivered by the property. This can be used to inform a processor that it should switch to values from an external graph when the user wants to retrieve more information about a value.<br><br>This property should be regarded as an "annotation", i.e. it does not have any impact on validation or other built-in SHACL features. However, selected tools may want to use this information. One implementation strategy would be to periodically fetch the values specified by the sh:node or sh:class shape associated with the property, using the property shapes in that shape, and add the resulting triples into the main query graph.<br><br>An example value is "https://query.wikidata.org/sparql".
    detailsEndpoint, "detailsEndpoint",
    /// `details graph`: Can be used to link a SHACL property shape with a SHACL node expression that produces the URIs of one or more graphs that contain further RDF triples for the value nodes delivered by the property. This can be used to inform a processor that it should switch to another data graph when the user wants to retrieve more information about a value.<br><br>The node expressions are evaluated with the focus node as input. (It is unclear whether there are also cases where the result may be different for each specific value, in which case the node expression would need a second input argument).<br><br>This property should be regarded as an "annotation", i.e. it does not have any impact on validation or other built-in SHACL features. However, selected tools may want to use this information.
    ...
    js, "js",
    /// `local constraint`: Can be set to true for those constraint components where the validation does not require to visit any other triples than the shape definitions and the direct property values of the focus node mentioned in the property constraints. Examples of this include sh:minCount and sh:hasValue.<br><br>Constraint components that are marked as such can be optimized by engines, e.g. they can be evaluated client-side at form submission time, without having to make a round-trip to a server, assuming the client has downloaded a complete snapshot of the resource.<br><br>Any component marked with dash:staticConstraint is also a dash:localConstraint.
    ...
    /// ``: 
    uriStart, "uriStart",
    /// `URI template`: Inserts a given value into a given URI template, producing a new xsd:anyURI literal.<br><br>In the future this should support RFC 6570 but for now it is limited to simple {...} patterns.
