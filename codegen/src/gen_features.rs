...
use anyhow::Context;
use toml::{value::{Table, Array}, Value};

use crate::{gen::CodegenContext, vocab_index::VocabIndex, helpers::io::{read_from_file, write_to_file}};
...
pub fn update_manifest(context: &CodegenContext) -> anyhow::Result<()> {
    ...
    let manifest_val = Value::from_str(&manifest_content)
    ...
    if let Value::Table(mut manifest_table) = manifest_val {
        ...
        write_to_file(&toml::to_string_pretty(&Value::Table(manifest_table))?, &manifest_path)
            .with_context(|| "Invalid manifest file")
    } else {
...
pub fn update_features_table(manifest_table: &mut Table, vocab_index: &VocabIndex) -> anyhow::Result<()> {
    ...
    if let Some(features) = features_opt {
        match features {
            ...
            _ => { return Err(anyhow::anyhow!("Invalid features table in manifest")); }
        }
    } else {
...
pub fn compute_features_table(vocab_index: &VocabIndex) -> Table {
    ...
    for vocab in vocab_index.index.values() {
        ...
        table.insert(feature_dataset_for(&vocab.safe_prefix), dataset_dep_features.clone());
    }
    table
}
