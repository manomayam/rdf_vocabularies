...
OntoInfo = namedtuple("OntoInfo", ["prefix_term", "prefix", "sanitized_prefix", "ns_base_iri", "onto_file_path"])
...
def get_ns_mod(g: rdflib.Graph, ns_base: str) -> str:
    term_names = sorted(get_ns_term_names(g, ns_base))
...
def get_ns_mod_from_file(gfp: str, ns_base: str, format=None) -> str:
    d = rdflib.Dataset()
    d.parse(str(gfp), format=format)
    return get_ns_mod(d.graph(ns_base), ns_base)
...
def gen_ns_mods(onto_infos: List[OntoInfo], repo_root_dir: str):
    ...
    for onto_info in onto_infos:
        ...
        ns_mod_content = get_ns_mod_from_file(onto_info.onto_file_path, onto_info.ns_base_iri, format='nquads')
...
{sep.join([
    f'#[cfg(feature = "vocab-{info.sanitized_prefix}")]{NEW_LINE}pub mod {info.sanitized_prefix};'
...
def get_onto_info_from_onto_file(index_graph: rdflib.Graph, onto_dir: str, onto_fie_name: str):
    ...
    if len(pref_triples) == 0:
        return OntoInfo(None, None, None, None, None)
    
    prefix_term = pref_triples[0][0]
    prefix = str(list(index_graph.triples((prefix_term, rdflib.URIRef("http://www.w3.org/ns/rdfa#prefix"), None)))[0][2])
    sanitized_prefix = sanitize_name(prefix)
    ns_base_iri = list(index_graph.triples((prefix_term, rdflib.URIRef("http://www.w3.org/ns/rdfa#uri"), None)))[0][2]
    return OntoInfo(prefix_term, prefix, sanitized_prefix, ns_base_iri, os.path.join(onto_dir, onto_fie_name))
